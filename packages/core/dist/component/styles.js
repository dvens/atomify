...
*/
export const generateStyles = (target, options) => {
    const style = options.style;
    const styles = options.styles;
    if (style && styles)
        throw new Error(`Component: ${target.__nodeName} can only have on type of style property: styles or style`);
    if (style)
        return style;
    if (styles)
        return styles.join('');
...
*/
export const generateGeneralStyling = (target, style) => {
